...
// Sort to nearest neighbour
TGraph* SortNearest(TGraph *graph) {

  double *x = graph->GetX();
  double *y = graph->GetY();
  int length = graph->GetN();

  // The sorted arrays of x and y
  ...
  // Move through each point going the shortest distance
  // Start at the first point
  int i = 0;
  // Count how many points we've gone through
  int counter = 0;
  while (counter < length) {
    ...
    // Calculate the shortest distance to any other point
    double shortest = 1.E9;
    // Save the index of the shortest distance
    int index = -1;
    //for (int j = 0; j < length; ++j) {
    for (int j = i-3; j < i+3; ++j) {
      //std::cout << "point " << i << " to " << j << std::endl;
      double xtwo = x[j];
      double ytwo = y[j];
      ...
      // Check that we haven't been on this point before
      bool found = false;
      for (int k = 0; k < i; ++k) {
        //std::cout << "checking " << j << " against " << k << std::endl;
        if (x[j] == sortedx[k]) {
      ...
      double dist = fabs(ydist);
      /*
         std::cout << xone << " to " << xtwo << std::endl;
         std::cout << yone << " to " << ytwo << std::endl;
         std::cout << dist << std::endl;
         std::cout << "***" << std::endl;
         */
      //std::cout << i << " " << j << " = " << xdist << " " << ydist << std::endl;
      if (dist < shortest) {
        shortest = dist;
        index = j;
      }
    ...
    std::cout << "Shortest distance: " << shortest << " between point " << i << " and " << index << std::endl;
    std::cout << y[i] << " " << y[index] << std::endl;

    // Update so that the next point is the closest point to the current
    sortedx[i] = xone;
    sortedy[i] = yone;
    i = index;
  ...
  for (int i = 0; i < length; ++i) {
  std::cout << sortedx[i] << " " << sortedy[i] << std::endl;
  }
  */

  // Delete the distance vector
  /*
     for (int i = 0; i < length; ++i) {
     delete distance[i];
     }
  ...
  // Make the new TGraph
  // Copy the graph to get colours etc
  TGraph *returned = new TGraph(*graph);
  // And then set the X
  for (int i = 0; i < length; ++i) {
    returned->SetPoint(i, sortedx[i], sortedy[i]);
  }
...
TGraph **GetSK() {
  ...
  //sk[0] = SortNearest(g_one); 
  //sk[1] = SortNearest(g_two); 
  //sk[2] = SortNearest(g_three); 
